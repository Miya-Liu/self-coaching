...
## Maintainers (two-repo workflow)

Public `main` stays lean. Full tests, integration snapshots, and internal tracking live in a **private** companion repo (`self-coaching-dev`). Use `bash scripts/setup-dev-remote.sh` once, then `bash scripts/promote-to-public.sh --push` to sync allowlisted paths to public `main`. Details are in `scripts/DEV_WORKFLOW.md` on the private dev tree (not published on public `main`).

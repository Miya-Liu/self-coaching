#!/usr/bin/env bash
# Shared helpers for self-tuning/pipelines/*/run.sh (sourced, not executed directly).
# Default: POST to TRAINER_BASE_URL (AERL; see self-tuning/pipelines/registry.yaml service.url).
...
training_skill_root() {
  # Caller is self-tuning/pipelines/<id>/run.sh → three levels up to modes/self-coaching root.
...
training_load_env() {
  ...
  if [[ -f "${root}/self-tuning/services/.env" ]]; then
    set -a
    # shellcheck disable=SC1090
    source "${root}/self-tuning/services/.env"
    set +a
  fi

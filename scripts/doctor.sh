...
script_files=(
  "${ROOT}"/scripts/*.sh
  "${ROOT}"/self-coaching-training/pipelines/*/run.sh
  "${ROOT}"/self-coaching-training/pipelines/_lib.sh
)
shopt -u nullglob
...
if git -C "${ROOT}" rev-parse --is-inside-work-tree >/dev/null 2>&1; then
  if git -C "${ROOT}" check-ignore -q self-coaching-training/services/.env; then
    emit PASS "gitignore.env" "self-coaching-training/services/.env is ignored"
  else
    emit FAIL "gitignore.env" "self-coaching-training/services/.env is NOT ignored — credential leak risk"
  fi
  # Sanity-check: example.env must NOT be ignored (it's the template users copy).
  if git -C "${ROOT}" check-ignore -q self-coaching-training/services/example.env; then
...
required_files=(
  ...
  "self-coaching-training/pipelines/_lib.sh"
  "self-coaching-training/pipelines/sft/run.sh"
  "self-coaching-training/pipelines/grpo/run.sh"
  "self-coaching-self-learning/SKILL.md"
  "self-coaching-self-play/SKILL.md"
  "self-coaching-evaluation/SKILL.md"
  "self-coaching-training/SKILL.md"

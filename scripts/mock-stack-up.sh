...
#
# Usage:
#   bash scripts/mock-stack-up.sh [data-dir] [--with-coaching]
#
...
if [[ "${WITH_COACHING}" -eq 1 ]]; then
  echo "==> Starting mock Coaching API on :${COACHING_PORT} (evaluate delegates to AgentEvals when MOCK_AGENTEVALS_URL set)"

...
"""Smoke test: pipeline self-play adapter (dry_run by default).

Usage:
  PIPELINE_DRY_RUN=1 python scripts/pipeline_self_play_smoke.py
  PIPELINE_INTEGRATION_TESTS=1 PIPELINE_SERVICE_URL=http://host:8001 python scripts/pipeline_self_play_smoke.py
"""
...
from services.adapters.selfplay_pipeline_adapter import build_self_play_pipeline_engine  # noqa: E402
...
def main() -> int:
    ...
    if not batch.get("proceed"):
        print("pipeline_self_play_smoke: FAIL batch did not proceed", file=sys.stderr)
        return 1
    ...
    if not suite.get("proceed"):
        print("pipeline_self_play_smoke: FAIL suite did not proceed", file=sys.stderr)
        return 1

    print("pipeline_self_play_smoke: PASS")
    return 0

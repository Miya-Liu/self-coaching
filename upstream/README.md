...
Worktrees for experiments still live under the coaching root: `worktrees/<experiment_id>/` (see `modes/skill/SKILL.md`).

**Optional legacy layout:** you may clone into `upstream/autoresearch/` here; that path is gitignored and never committed.

**Without autoresearch:** use AERL HTTP pipelines only (`modes/skill/self-tuning/pipelines/`, `TRAINER_BASE_URL`) — no `AUTORESEARCH_ROOT` required.
